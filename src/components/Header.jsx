...
//         {/* Profil */}
//         <div className="relative">
//           <img src={user.imageprofil_url || "/default-profile.png"} alt="Profile" />
...
const Header = () => {
  ...
  const [notifications, setNotifications] = useState([]);
  const [avatar, setAvatar] = useState(user?.imageprofil_url || "/default-profil.jpg");
  ...
  const handleChangeAvatar = async (e) => {
    ...
    try {
      ...
      // Utiliser URL complète renvoyée par Django/Cloudinary
      const newAvatar = res.data.imageprofil_url || res.data.imageprofil || "/default-profil.jpg";
      setAvatar(newAvatar);

      if (setUser) {
        setUser({ ...user, imageprofil_url: newAvatar });
      }
    } catch (err) {
      console.error("Erreur upload avatar:", err);
    }
  ...
  // Met à jour avatar si user change
  useEffect(() => {
    setAvatar(user?.imageprofil_url || "/default-profil.jpg");
  }, [user]);
